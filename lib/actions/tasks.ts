...
export async function createTask(familyId: string, title: string, assignee?: string, list_id?: string, parent_id?: string): Promise<Task> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      // position will be auto-generated by DB default logic or appended at 0 for now.
      // Wait, there is no position default in DB. Let's just set it to Date.now() for default ordering
      position: Date.now()
    })
    .select()
    .single();
