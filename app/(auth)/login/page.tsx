...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] flex flex-col items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md text-center border border-[#C8D4E8]">
        ...
        <div className="mb-8 flex flex-col items-center gap-3">
          <Image
            ...
            priority
          />
          <h1 className="text-4xl font-black text-[#1B2A4A] tracking-wide">FamilyOS</h1>
          <p className="text-[#4A5568]">מנהל המשימות המשפחתי החכם</p>
        </div>
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-3 bg-white border border-[#C8D4E8] text-[#1B2A4A] font-medium py-3 px-4 rounded-xl hover:bg-[#F4F7FB] hover:border-[#2E4A7A] transition-all shadow-sm group disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <span className="w-5 h-5 border-2 border-[#1B2A4A] border-t-transparent rounded-full animate-spin"></span>
          ) : (
